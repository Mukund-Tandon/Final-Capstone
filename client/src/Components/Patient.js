...
class Patient extends Component {
    ...
    async loadcontract(){
        ...
        console.log("loading contract -2");
        const networkId = await web3.eth.net.getId();
        console.log("loading contract -3");
        var deployedNetwork = healthRecord.networks[networkId];
        console.log("loading contract -4");
    ...
    async loadPatient (){
        ...
        console.log("inside loading apatient");
        ...
        console.log("loading patient complete");
    }
    ...
    updateFileHash = async (name, type, ipfsHash) => {
        try {
          // sending transaction and storing result to state variables
          const res = await this.contract.methods.addUserFiles(name, type, ipfsHash).send({ "from": this.accounts[0] });
          console.log(res);
          if (res) {
            console.log("File upload successful");
          } else {
    ...
    uploadFile = async (event) => {
        ...
        try {
          // Replace 'YOUR_API_KEY' and 'YOUR_API_SECRET' with your Pinata API key and secret
          ...
          const { cid } = await pinata.pinFileToIPFS(this.state.buffer);
    
          // Update the file hash in the blockchain
          await this.updateFileHash(this.state.file.name, this.state.file.type, cid);
